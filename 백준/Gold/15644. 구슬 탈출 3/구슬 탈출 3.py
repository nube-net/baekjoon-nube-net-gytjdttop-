...
while cnt < 11:
    ...
    for _ in range(l):
        ...
        R = [loc[0], loc[1]]
        B = [loc[2], loc[3]]
        direction_ = []  # 방향(문자열) 저장 예정
        if len(loc) > 4:
            direction_ = list(loc[4:])

        for i in range(4):
            goal_b, goal_r = False, False
            r, b, direction = copy.deepcopy(R), copy.deepcopy(B), copy.deepcopy(direction_)
            ...
            if (R[0]+dx[i], R[1]+dy[i]) == tuple(B):  # 바로 옆에 파란 구슬 있을 경우, 파란 구슬 먼저 이동
                ...
                if goal_b:
                    ...
                    ans = True
                    direction.append(dd[i])
                    break
                else:
                    if tuple(r + b) not in check:
                        direction.append(dd[i])
                        Q.append(tuple(r + b+direction))
            else:
                while key_r or key_b:
                    if key_r:
                        if (r[0] + dx[i], r[1] + dy[i]) in tmp:
